...
class HierarchicalMultiTaskModel(nn.Module):
    def __init__(self, num_species, num_families, num_traits, num_seg_classes, model_name='swin_base_patch4_window7_224.ms_in22k'):
        ...
        # Tham số học trọng số loss (Uncertainty Weighting)
        self.log_var_species = nn.Parameter(torch.zeros(1), requires_grad=True)
        self.log_var_family = nn.Parameter(torch.zeros(1), requires_grad=True)
        ...
        self.log_var_s_consistency = nn.Parameter(torch.zeros(1), requires_grad=True)
        self.log_var_f_consistency = nn.Parameter(torch.zeros(1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        return {
            ...
            'log_vars': {
                ...
                'family': self.log_var_family,
                'trait': self.log_var_trait,
                'seg': self.log_var_seg,
                's_consistency': self.log_var_s_consistency,
                'f_consistency': self.log_var_f_consistency
            }
        }
...
def evaluate(model, loader, split_name, criteria, args):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"Evaluating {split_name}"):
            ...
            # --- Tính loss tổng thể ---
            total_loss = ((torch.exp(-log_vars['species']) * loss_species + log_vars['species']) +
                          (torch.exp(-log_vars['family']) * loss_family + log_vars['family']) +
                          (torch.exp(-log_vars['trait']) * loss_trait + log_vars['trait']) +
                          (torch.exp(-log_vars['seg']) * loss_seg + log_vars['seg']) +
                          (torch.exp(-log_vars['s_consistency']) * loss_s_consistency + log_vars['s_consistency']) +
                          (torch.exp(-log_vars['f_consistency']) * loss_f_consistency + log_vars['f_consistency']))
...
def main(args):
    ...
    # --- Load Data, Metadata, Constraint Matrices ---
    ...
    if not args.evaluate_only:
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch+1}")):
                ...
                # Áp dụng Uncertainty Weighting
                loss = ((torch.exp(-log_vars['species']) * loss_species + log_vars['species']) +
                        (torch.exp(-log_vars['family']) * loss_family + log_vars['family']) +
                        (torch.exp(-log_vars['trait']) * loss_trait + log_vars['trait']) +
                        (torch.exp(-log_vars['seg']) * loss_seg + log_vars['seg']) +
                        (torch.exp(-log_vars['s_consistency']) * loss_s_consistency + log_vars['s_consistency']) +
                ...
                if not (torch.isnan(loss) or torch.isinf(loss)):
                    ...
                    if batch_idx > 0 and batch_idx % 500 == 0:
                        ...
                        print(f"    Raw Losses -> S:{loss_species.item():.2f}, F:{loss_family.item():.2f}, T:{loss_trait.item():.2f}, G:{loss_seg.item():.2f}, SC:{loss_s_consistency.item():.2f}, FC:{loss_f_consistency.item():.2f}")
                        print(f"    Weights    -> S:{torch.exp(-log_vars['species']).item():.2f}, F:{torch.exp(-log_vars['family']).item():.2f}, T:{torch.exp(-log_vars['trait']).item():.2f}, G:{torch.exp(-log_vars['seg']).item():.2f}, SC:{torch.exp(-log_vars['s_consistency']).item():.2f}, FC:{torch.exp(-log_vars['f_consistency']).item():.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Fish-Vista: Hierarchical Training with Multi-level Consistency')
    parser.add_argument('--run-name', type=str, default='hierarchical_exp_1')
    ...
    parser.add_argument('--s-constraint-path', type=str, default='species_trait_constraints.pt', help="Path to species-trait constraint matrix.")
    parser.add_argument('--f-constraint-path', type=str, default='family_trait_constraints.pt', help="Path to family-trait constraint matrix.")
